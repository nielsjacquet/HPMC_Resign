...
serverPath="/Volumes/Macintosh HD-1/Users/Shared/HPMC"
...
#iosJsPath="$DIR/Resources/HybridJS"                                ##path to the Hybrid folder
toBeSignedFolder="$DIR/ToBeSigned"                                  ##path for the ipas that needs to signed
tempFolder="$serverPath/_TEMP"                                             ##Temp folder for extract the ipa etc
entitlements="$tempFolder/entitlements.txt"                          ##temp path used for the entitlements
payloadFolder="$tempFolder/Payload"                           ##payloadFolder
extractFile="$serverPath/test.txt"                                         ##used for the extraction of the devices in the provisioningProfile
LC_ALL=C

printf "${RED}Entitlement DEBUGGING: $entitlements${NC}\n"

##enter Endpoints
printf "${GREEN}Please enter the apiUrl without https:// ${NC}\n"
read apiUrl
printf "${GREEN}Please enter the apiTokenUrl https:// ${NC}\n"
read apiTokenUrl
...
function destinationFolderCheck {
  signedIpasFolder="$serverPath/SignedIpas"
  ...
  ipaArchiveFolder="$serverPath/OriginalipaArchive"
...
function destinationFolderCreation {
  ...
  destinationFolder="$serverPath/SignedIpas/Apps/$fixDate - $payloadAppCleaned"
...
function getUsedEndpoints {
  printf "${YELLOW}Getting the Used Endpoint${GREEN}\n"
  usedEndpoints="$destinationFolder/Endpoints_$ogIpa.txt"
  printf "${RED}getUsedEndpoints DEBUGGING destinationfoldr: $destinationFolder ${NC}\n"
  printf "${RED}getUsedEndpoints DEBUGGING: $usedEndpoints ${NC}\n"
  echo "apiUrl: https://$apiUrl" >> "$usedEndpoints"
  echo "apiTokenUrl: https://$apiTokenUrl" >> "$usedEndpoints"
}
...
function editEndpoints {
  printf "${GREEN}Editing the Endpoints in the Config.plist${NC}\n"
  cd "$payloadFolder"
  payloadApp=$(ls | grep '.app')
  printf "${RED}DEBUGGING EDITENDPOINTS payloadApp: $payloadApp ${NC}\n"
  cd "$payloadFolder/$payloadApp"
  configPlist=$(ls | grep 'Config.plist')
  printf "${RED}DEBUGGING EDITENDPOINTS configPlist: $configPlist ${NC}\n"
  LC_ALL=C sed -i '' 's/btpm-int-tokenapi-webapp.azurewebsites.net/'$apiTokenUrl'/g' $configPlist     ## Replace the apiTokenUrl
  LC_ALL=C sed -i '' 's/btpm-int-api-webapp.azurewebsites.net/'$apiUrl'/g' $configPlist               ## Replace the apiUrl
  extractVersion
}

function extractVersion {                                                       ##extract the app version for naming sceme
  infoPlist="$tempFolder/Payload/$payloadApp/info.plist"
  plutil -convert xml1 "$infoPlist"
  printf "${GREEN}Extracting the app version${NC}\n"
  buildVersionRude=$(destinationFolderCheck $infoPlist | grep -A1 "CFBundleVersion")
...
function cleanup {
  ...
  dateDir="$serverPath/OriginalipaArchive/$fixDate - $payloadAppCleaned"
  function DateDir {
    if [[ -d "$dateDir" ]]
    then
